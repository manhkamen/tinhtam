...
export default function App() {
  ...
  const [savedData, setSavedData] = useState<{ chapter: string; time: string; userName: string; phapdanh: string; address: string } | null>(null);
  const [userName, setUserName] = useState<string>("");
  const [phapdanh, setPhapdanh] = useState<string>("");
  const [address, setAddress] = useState<string>("");
  const [isEditing, setIsEditing] = useState<boolean>(false);
  const [history, setHistory] = useState<Array<{ chapter: string; time: string; date: string }>>([]);
...
Nam mô A Di Đà Phật. (3 lần)`}
            ...
            <div style={{ width: '100%', maxWidth: 700, margin: "20px auto", display: 'flex', flexDirection: 'column', gap: '15px' }}>
              {savedData && !isEditing ? (
                <div style={{ backgroundColor: 'rgba(255, 255, 255, 0.15)', padding: '20px', borderRadius: '15px', textAlign: 'center', boxShadow: '0 4px 15px rgba(0,0,0,0.2)' }}>
                  <p style={{ margin: 0, fontWeight: 'bold', fontSize: '22px' }}>Đã lưu: {savedData.chapter}</p>
                  <p style={{ margin: '10px 0 0 0', fontSize: '16px', opacity: 0.9 }}>Thời gian: {savedData.time}</p>
                  <button
    ...
    days.push(
      ...
      >
        <div style={{
          ...
          alignItems: 'center',
          justifyContent: 'center',
          backgroundColor: isMarked ? '#ff9800' : (isToday ? 'rgba(255,255,255,0.2)' : 'transparent'),
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
        <button onClick={prevMonth} style={{ background: 'none', border: 'none', color: '#fff', cursor: 'pointer', fontSize: '20px' }}>&lt;</button>
        <div style={{ fontWeight: 'bold' }}>{monthNames[month]} {year}</div>
        <button onClick={nextMonth} style={{ background: 'none', border: 'none', color: '#fff', cursor: 'pointer', fontSize: '20px' }}>&gt;</button>
      </div>

      <div style={{ display: 'flex', flexWrap: 'wrap', textAlign: 'center', fontWeight: 'bold', fontSize: '12px', opacity: 0.7, marginBottom: '5px' }}>
